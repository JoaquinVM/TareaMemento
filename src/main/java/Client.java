public class Client {
    public static void main(String[] args) {
        ...
        caretaker.addMemento( "state2", originator.createMemento() ); // creamos memento [0] --> State2

        concreteObject = new ConcreteObject("Juegos","State3");
        originator.setMemento(concreteObject);

        caretaker.addMemento("state3", originator.createMemento() ); // creamos memento [1] --> State3

        concreteObject = new ConcreteObject("Juegos","State4");
        originator.setMemento(concreteObject);

        originator.restoreFromMemento( caretaker.getMemento("state2") );
    }
}